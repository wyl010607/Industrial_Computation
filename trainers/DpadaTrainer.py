...
class DpadaTrainer(AbstractTrainer):
    """
    ...
    def train_one_epoch(self, s_data_loader, t_data_loader):
        ...
        with tqdm(total=len(t_data_loader), leave=False) as pbar:
            ...
            for i, ((s_x, s_y), (t_x, t_y)) in enumerate(zip(s_data_loader, t_data_loader)):
                ...
                domain_pred, class_pred,high_feature= self.model(inputs)
                
                # 计算高层特征之间的 MSE 距离
                mse_loss = nn.MSELoss()(high_feature.narrow(0, 0, batch_num), high_feature.narrow(0, batch_num, batch_num))
                
                # 调整 MSE 距离并转换为损失值
                mse_loss = 1 - torch.exp(mse_loss*mse_loss / (2*C*C))
                ...
                loss = classification_loss + 0.5*adversarial_loss + mse_loss
    ...
    def evaluate(self, data_loader, metrics, **kwargs):
        ...
        for i, (x, y) in enumerate(data_loader):
            ...
            domain_label,class_pred,high_features = self.model(x)
