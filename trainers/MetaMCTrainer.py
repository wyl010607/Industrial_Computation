...
class MetaMCTrainer(AbstractTrainer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.m_model = copy.deepcopy(self.model)
        self.optimizer = optimizer(self.model.parameters())
        self.meta_optimizer = metaoptimizer(self.m_model.parameters())
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            self.update_trainer_params["metaoptimizer_params.params"] = self.model.parameters()
    ...
    def train_one_epoch(self, train_loader, val_loader, *args, **kwargs):
        self.model.train()
        total_loss = 0
        query_loss = 0.0
        # m_model = copy.deepcopy(self.model)
        self.update_trainer_params["optimizer_params.params"] = self.m_model.parameters()

        for input_data, labels in enumerate(train_loader):
            self.meta_optimizer.zero_grad()
            input = input_data.float().to(self.device)

            for task_support in input:
                # 多尺度patch列表
                patch_num_dist_list, patch_size_dist_list, patch_num_mx_list, patch_size_mx_list, recx = self.m_model(task_support)
                loss = 0.0
        ...
        temp_param = self.m_model.parameters()

        for input_data, labels in enumerate(val_loader):
            self.optimizer.zero_gard()
            input = input_data.float().to(self.device)

            for task_query in input:
                patch_num_dist_list, patch_size_dist_list, patch_num_mx_list, patch_size_mx_list, recx = self.m_model(task_query)
                loss = 0.0
                cont_loss1, cont_loss2 = self.anomaly_score(patch_num_dist_list, patch_size_mx_list, win_size=self.window,
                                                            train=1, temp=1)
                cont_loss_1 = cont_loss1 - cont_loss2
                loss -= self.hyper_c * cont_loss_1
                # 计算投影差异损失
                cont_loss12, cont_loss22 = self.anomaly_score(patch_num_mx_list, patch_size_dist_list, win_size=self.window,
                                                            train=1, temp=1)
                cont_loss_2 = cont_loss12 - cont_loss22
                loss -= self.hyper_c * cont_loss_2
                patch_num_loss, patch_size_loss = self.anomaly_score(patch_num_dist_list, patch_size_dist_list,
                                                                    win_size=self.window, train=1, temp=1)
                # 表征差异损失归一化
                patch_num_loss = patch_num_loss / len(patch_num_dist_list)
                patch_size_loss = patch_size_loss / len(patch_num_dist_list)
                # 关联差异
            ...
            # 元学习参数迁移
            with torch.no_grad():
                for param_copy, param_origin in zip(temp_param, self.model.parameters()):
                    param_origin.grad.copy_(param_copy.grad)
            self.optimizer.step()
