...
class MetaMCTrainer(AbstractTrainer):
    ...
    def maml_train(self, support, query, meta_optimizer):
        # 一阶段内循环更新，使用支持集数据进行训练
        m_model = copy.deepcopy(self.model)
        total_loss = 0
        query_loss = 0.0
        for input_data, labels in enumerate(support):
            meta_optimizer.zero_grad()
            input = input_data.float().to(self.device)

            for task_support in input:
                # 多尺度patch列表
                patch_num_dist_list, patch_size_dist_list, patch_num_mx_list, patch_size_mx_list, recx = m_model(
                    task_support)
                loss = 0.0
                cont_loss1, cont_loss2 = anomaly_score(patch_num_dist_list, patch_size_mx_list,
                                                            win_size=self.win_size, train=1, temp=1)
                cont_loss_1 = cont_loss1 - cont_loss2
                loss -= self.patch_mx * cont_loss_1
                # 计算投影差异损失
                cont_loss12, cont_loss22 = anomaly_score(patch_num_mx_list, patch_size_dist_list,
                                                              win_size=self.win_size, train=1, temp=1)
        ...
        temp_param = m_model.parameters()
        # 一阶段内循环更新，使用支持集数据进行训练
        for input_data, labels in enumerate(query):
    ...
    def train_one_epoch(self, train_loader, test_loader, epoch, time_now, train_len, support, query, *args, **kwargs):
