...
class AbstractTrainer(ABC):
    ...
    def train(
        ...
        **kwargs,
        ...
        tmp_state_save_path = os.path.join(self.model_save_dir_path, "temp_WAEL.pkl")
        epoch_result_list = []
        min_loss = 1e+8
        ...
        for epoch in range(self.epoch_now, self.max_epoch_num):
            ...
            train_loss = self.train_one_epoch(train_loader, val_loader, self.epoch_now, time_now, train_steps)  # 训练一个epoch
            self.epoch_now += 1
            # evaluate
            print(f"验证阶段:Epoch {epoch}")
            if val_loader is not None:
                val_loss, val_result = self.evaluate(val_loader, metrics, mode='val')
                # if self.epoch_now==self.max_epoch_num:
                epoch_result_list.append([epoch, train_loss, val_loss, val_result])
                ...
                if stop_improve_count >= self.early_stop_patience:
                    break
                else:
    ...
    def test(self, val_loader, test_loader, metrics=("prec", "rec", "f1", "auc"), *args, **kwargs):
        # load best model
        self.model.load_state_dict(torch.load(self.model_save_path))
        # evaluate on test set
        print("模型测试：")
        self.epoch_now = self.epoch_now+1
        test_loss, self.test_result = self.evaluate(test_loader, metrics)
        val_loss, self.val_result = self.evaluate(val_loader, metrics)
        p, r, f, auc = get_score(self.test_result, self.val_result, mode='test')
        test_result = {"loss": test_loss, "precision": p, "recall":r, "f1_score":f, "auc_score":auc}
