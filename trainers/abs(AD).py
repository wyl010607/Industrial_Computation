...
from trainers.DCTrainer import DCTrainer
...
class AbstractTrainer(ABC):
    ...
    def train(
        ...
        **kwargs,
        ...
        for epoch in range(self.epoch_now, self.max_epoch_num):
            ...
            # train
            self.update_model_params["epoch"] = self.epoch_now
            train_loss = self.train_one_epoch(train_data_loader,eval_data_loader, self.epoch_now)  # 训练一个epoch
            # DCTrainer
            # lr = self.kwargs.get("lr")
            # DCTrainer.adjust_learning_rate(epoch + 1, lr)
            self.epoch_now += 1
    ...
    def test(self, test_data_loader, metrics=("prec", "recall", "f1", "auc"), *args, **kwargs):
        """
