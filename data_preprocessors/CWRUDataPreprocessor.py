...
class CWRUDataPreprocessor(AbstractDataPreprocessor):
    """
    ...
    def load_data(self):
        ...
        if(self.data_dimension == "1D"):
            s_path = self.data_path + "/Drive_end_" + str(self.s_load) + "/"
            t_path = self.data_path + "/Drive_end_" + str(self.t_load) + "/"

            # 加载源域数据
            self.s_data, self.s_label = load_mat(s_path, self.s_load, self.s_label_set, self.name_dict, self.feature_name,
                                                self.data_length, self.window)

            # 加载目标域数据
            self.t_data, self.t_label = load_mat(t_path, self.t_load, self.t_label_set, self.name_dict,
                                                self.feature_name, self.data_length, self.window)
        if(self.data_dimension == "DWT"):
            s_path = self.data_path+"/DWT/Drive_end_"+str(self.s_load)
            t_path = self.data_path+"/DWT/Drive_end_"+str(self.t_load)
