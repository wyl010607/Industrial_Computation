...
class GDNSDataPreprocessor(AbstractDataPreprocessor):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.update_dataset_params["down_ration"] = self.down_ratio
        self.kwargs = kwargs
    ...
    def preprocess(self):
        ...
        self.test_df = pd.DataFrame(d_test_x, columns=self.test.columns)
        self.test_df['attack'] = d_test_labels
        self.train_df = train_df.iloc[2160:]
    ...
    def split_data(self, preprocessed_data):
        ...
        net = NetStruc('swat', list_path, self.train_df)
        feature_map = net.get_feature()
        fc_edge_index = net.graph_struct()
        edge_index_sets=[]
        edge_index_sets.append(fc_edge_index)
        self.update_model_params["edge_index_sets"] = edge_index_sets
        self.update_model_params["node_num"] = len(feature_map)
        self.train_df, self.test_df = net.construct_data(self.train_df, feature_map, labels=0), net.construct_data(self.test_df, feature_map, labels=0)
