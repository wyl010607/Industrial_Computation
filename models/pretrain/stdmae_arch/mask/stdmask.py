...
class STDMask(nn.Module):
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        if mask:
            ...
            hidden_states_unmasked = self.encoder(
                encoder_input
            )  # B,  P,N, d/# B, N, P, d
            if self.spatial:
                hidden_states_unmasked = hidden_states_unmasked.transpose(
                    -2, -3
                )  # B, N, P, d
            hidden_states_unmasked = self.encoder_norm(hidden_states_unmasked).view(
                batch_size, num_nodes, -1, self.embed_dim
    ...
    def decoding(self, hidden_states_unmasked, masked_token_index):
        ...
        hidden_states_unmasked = self.enc_2_dec_emb(
        ...
        if self.spatial:
            ...
            (
                ...
                num_nodes,
                _,
            ) = hidden_states_unmasked.shape  # B,P, N,  d
            unmasked_token_index = [
                i
                for i in range(0, len(masked_token_index) + num_nodes)
                if i not in masked_token_index
            ]
            hidden_states_masked = self.pos_mat[
                :, masked_token_index, :, :
            ]  # B, N*r,P,  d
            hidden_states_masked = hidden_states_masked.transpose(
                -2, -3
            )  # B, P, N*r, d
            ...
            hidden_states_masked += self.mask_token.expand(
                batch_size,
                num_time,
                len(masked_token_index),
                hidden_states_unmasked.shape[-1],
            )  # B, P, N*r, d
            hidden_states_unmasked += self.pos_mat[
                :, unmasked_token_index, :, :
            ].transpose(
                -2, -3
            )  # B,  P,N*(1-r), d
            hidden_states_full = torch.cat(
                [hidden_states_unmasked, hidden_states_masked], dim=-2
            )  # B, P, N, d
            ...
            unmasked_token_index = [
                i
                for i in range(0, len(masked_token_index) + num_time)
                if i not in masked_token_index
            ]
    ...
    def get_reconstructed_masked_tokens(
        ...
        if self.spatial:
            ...
            reconstruction_masked_tokens = reconstruction_full[
                :, :, len(unmasked_token_index) :, :
            ]  # B, P, r*N, L
            reconstruction_masked_tokens = reconstruction_masked_tokens.view(
                batch_size, num_time, -1
            )  # B, P, r*N*L
            ...
            label_masked_tokens = (
                label_full[:, masked_token_index, :, :].transpose(1, 2).contiguous()
            )  # B,  P,N*r, L
            ...
            label_masked_tokens = label_full[
                :, :, masked_token_index, :
            ].contiguous()  # B, N, r*P, d
            label_masked_tokens = label_masked_tokens.view(
                batch_size, num_nodes, -1
            ).transpose(
                1, 2
    ...
    def forward(self, history_data: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.mode == "pre-train":
            ...
            (
                ...
                unmasked_token_index,
                masked_token_index,
            ) = self.encoding(history_data)
            # decoding
            reconstruction_full = self.decoding(
                hidden_states_unmasked, masked_token_index
            )
