...
from models.COMTPGNET.fsnet_ import DilatedConvEncoder
...
class TSEncoder(nn.Module):
    def __init__(self, device, num_nodes, supports, history_len, in_dim=2, out_dim=12, residual_channels=32,
                 dilation_channels=32, skip_channels=256, end_channels=512, emb_channels=16, blocks=4, layers=2,
                 dropout=0.3, mask_mode='binomial', gamma=0.9):
        super().__init__()
        ...
        self.end_conv_0 = nn.Conv2d(in_channels= 49,
                                    out_channels= 1,
                                    kernel_size=(1, 1),
                                    bias=True)
    ...
    def forward(self, x, time_emb, mask):
        ...
        x = self.end_conv_0(x.transpose(1,3)).transpose(1,3)
