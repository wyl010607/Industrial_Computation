...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layer_stack = nn.ModuleList([
            EncoderLayer_stamp(num_route, history_len, n_attr, n_hid, dis_mat, attn, STstamp, drop_prob, n_c, enc_spa_mask, enc_tem_mask)
            for _ in range(1)
        ])
...
class STAGNN_stamp(nn.Module):
    def __init__(
        self, n_layer, n_attr, n_hid, reg_A, circle, drop_prob, n_c, a, n_mask, adj_mx, CE, LE, SE, TE, attn, STstamp, T4N,
        ...
        self.encoder = Encoder(num_route, history_len, n_attr, n_hid, adj_mx, attn, STstamp, drop_prob, n_c, enc_spa_mask, enc_tem_mask)
