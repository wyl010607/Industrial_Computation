...
class ASTGCN(torch.nn.Module):
    ...
    def __init__(
        ...
        kernel_size=3,
        stride=1,
        first_time_conv=False,
        first_time_conv_kernel_size=None,
        ...
        **kwargs,
        ...
        if first_time_conv:
            assert (
                first_time_conv_kernel_size is not None
                and first_time_conv_stride is not None
            ), "Kernel size and stride must be provided for first time convolution"
            ...
            history_len_after_conv = (
                history_len - first_time_conv_kernel_size
            ) // first_time_conv_stride + 1
            history_len = history_len_after_conv

        self.adj_mx = nn.Parameter(GCN.build_adj_matrix(adj_mx, adj_type="cheb", K=K))
        ...
        for i in range(1, num_block + 1):
            ...
            history_len = (history_len + 2 * padding - kernel_size) // stride + 1
            f_in = num_time_filter
