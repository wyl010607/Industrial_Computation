...
class GCN(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # self.adj_mx = self.build_adj_matrix(
        #     np.load(adj_path), device, adj_type=adj_type, **kwargs
        # )
        self.adj_mx = self.build_adj_matrix(
            np.load(adj_path), adj_type=adj_type, **kwargs
        )
        # self.gcn_list = self._build_gcn_network(
        #     input_size, hidden_sizes, step_num_out, with_res
        # )
    ...
    @staticmethod
    def build_adj_matrix(adj, adj_type=None, **kwargs):
        ...
        return torch.from_numpy(cheb_array)
