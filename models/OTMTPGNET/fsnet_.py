...
class gcn(nn.Module):
    def __init__(self,c_in,c_out,dropout, kt, support_len=3,order=2):
...
class SamePadConv(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels, kt, supports_len, kernel_size, dropout, dilation=1, groups=1, gamma=0.9):
        ...
        self.gconv = gcn(out_channels, in_channels, dropout, kt, support_len=supports_len)
        ...
        self.cof = torch.zeros((1, self.kt + 1)).float().cuda()
        self.time_emd_new = torch.zeros((1, self.kt + 1)).float().cuda()
        self.trigger = 0
        self.trigger_g = 0
        self.tau = 0.75
        self.tau2 = 0.75
    ...
    def change_graph(self):
        if not self.training:
            if  torch.norm(self.cof - self.time_emd_new)> self.epsilon:
                self.trigger_g = 1
    ...
    def fw_chunks(self):
        ...
        if self.trigger == 1:
            ...
            #vv = v / sum(v)
            #vv = vv.unsqueeze(1).float().detach()
    ...
    def graphcof(self, time_emb):
        if self.trigger_g == 1 :
            self.trigger_g = 0
            q = self.time_emd_new.flatten()
            # read
            att = q @ self.GW
            att = F.softmax(att / 0.5)
            v, idx = torch.topk(att, 2)
            ...
            self.GW.data = normalize(self.GW.data)
            if self.sum > self.eta:
                self.cof = self.tau2 * self.time_emd_new + (1 - self.tau2) * old_w.view(time_emb.size())
            else:
                self.sum = self.sum + 1
            self.time_emd_new = time_emb
            return self.cof
        else:
            self.time_emd_new = time_emb
            return time_emb
    def forward(self, x, skip, supports, time_emb):
        ...
        if self.trigger == 1 :
            ...
            self.GW.data = normalize(self.GW.data)
            if self.sum > self.eta:
                time_emb = self.tau2 * time_emb + (1 - self.tau2) * old_w.view(time_emb.size())
            else:
                self.sum = self.sum + 1
        '''
        time_emb = self.graphcof(time_emb)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels, layers, kt, supports_len, kernel_size, dilation, dropout=0.3, gamma=0.9):
        super().__init__(),
        ...
        for i in range(self.layers):
            self.convs.append(SamePadConv(in_channels, out_channels, skip_channels, kt, supports_len, kernel_size, dilation=dilation, dropout = dropout, gamma=gamma))
            dilation *= 2
    def forward(self, x, skip, supports, time_emb):
        for i in range(self.layers):
            x = F.gelu(x)
...
class DilatedConvEncoder(nn.Module):
    def __init__(self, residual_channels, dilation_channels, skip_channels, blocks, layers, dropout, kt, supports_len, kernel_size, gamma):
        super().__init__()
