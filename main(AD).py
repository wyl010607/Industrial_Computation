...
def main(args):
    ...
    # epoch_results = trainer.train(train_dataloader, valid_dataloader, valid_dataloader)

    #USAD
    test_result, y_pred, y_true= trainer.test(test_dataloader)
    # GDN
    # metrics = ("prec", "recall", "f1", "auc")
    #val_result = trainers.GDNTrainer.evaluate(test_dataloader, metrics)
    # test_result = trainers.GDNTrainer.evaluate(test_dataloader,metrics)
    # f1,p,r,auc = trainers.GDNTrainer.get_score(test_result,val_result)
    # y_pred, y_true = test_result[0], test_result[2]
    # DCdetector
    #test_result, y_pred, y_true = trainer.test(test_dataloader)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_config_path",
        type=str,
        default="./config/train_config/USAD_train_config.yaml",
    ...
    parser.add_argument(
        "--data_config_path",
        type=str,
        default="./config/data_config/USADS_config.yaml",
        help="Config path of Data",
    )
    parser.add_argument("--model_name", type=str, default="USAD", help="Model name")
    parser.add_argument(
        "--model_save_path",
        type=str,
        default="./model_states/USAD.pkl",
