...
class LSTMDataset(AbstractDataset):
    ...
    def getDataloader(data_path, sub_dataset):
        ...
        for c in df_train.columns:
            ...
            if 's' in c:
                ...
                for n in np.arange(1,engine_numbers+1):
                    ...
                    sm = list(LSTMDataset.smooth(s, 0.98))
                    sm_list += sm
        ...
        train_indices = list(train_data[(train_data['rul'] >= ((window - 1)-minmax_dict['rul'+'min']) / (minmax_dict['rul'+'max'] - minmax_dict['rul'+'min'])) & (train_data['time'] > 10)].index)
        val_indices = list(val_data[(val_data['rul'] >= ((window - 1)-minmax_dict['rul'+'min']) / (minmax_dict['rul'+'max'] - minmax_dict['rul'+'min'])) & (val_data['time'] > 10)].index)
        ...
        train = LSTMDataset(train_indices, df_train,False)
        #print(len(train))
        val = LSTMDataset(val_indices, df_train,False)
        test = LSTMDataset(units, df_test,True)
        dataset_list = [train, test,val]
        os.environ["LOKY_MAX_CPU_COUNT"] = str(os.cpu_count())
        # LSTMDataset.clustering_operations(dataset_list)
