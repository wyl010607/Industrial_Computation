...
def main(args):
    ...
    task_support, task_query, train_dataset, train_sampler, train_shuffle, val_dataset, val_sampler, \
        val_shuffle, test_dataset, test_sampler, test_shuffle, thre_dataset, \
        thre_sampler, thre_shuffle = data_preprocessor.split_data()
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=batch_size, sampler=train_sampler, shuffle=train_shuffle, **data_config["dataloader_params"]
    )
    valid_dataloader = DataLoader(
        val_dataset, batch_size=batch_size, sampler=val_sampler, shuffle=val_shuffle, **data_config["dataloader_params"]
    )
    test_dataloader = DataLoader(
        test_dataset, batch_size=batch_size, sampler=test_sampler, shuffle=test_shuffle, **data_config["dataloader_params"]
    )
    thre_dataloader = DataLoader(
        thre_dataset, batch_size=batch_size, sampler=thre_sampler, shuffle=thre_shuffle, **data_config["dataloader_params"]
    )
    ...
    trainer = trainer_class(
        ...
        # dataset=train_config["optimizer_params"]["lr"],
    ...
    epoch_results = trainer.train(train_dataloader, test_dataloader, thre_dataloader, task_support, task_query)
    test_result, y_pred, y_true = trainer.test(train_dataloader, test_dataloader)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_config_path",
        type=str,
        default="./config/train_config/MetaMC_train_config.yaml",
    ...
    parser.add_argument(
        "--data_config_path",
        type=str,
        default="./config/data_config/MetaMC_config.yaml",
        help="Config path of Data",
    )
    parser.add_argument("--model_name", type=str, default="MCdetector", help="Model name")
    parser.add_argument(
        "--model_save_path",
        type=str,
        default="./model_states/MetaMCdetector.pkl",
    ...
    parser.add_argument(
        "--result_save_dir_path",
        type=str,
        default="./results/MetaMCdetector",
